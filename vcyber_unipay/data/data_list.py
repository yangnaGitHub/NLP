...
#                    break
            ...
            for data in self.datas:
                ...
                for content in (data[2]).split('#&#&#'):
                    ...
                    for exp_content in self.extend_by_sameword(content):
                        if isinstance(exp_content, list):
                            if exp_content not in self.quests:
                                self.quests.append(exp_content)
                                self.label_quest[label_id].append(exp_content)
                        else:
                ...
                for content in (data[3]).split('#&#&#'):
                    ...
                    for exp_content in self.extend_by_sameword(content):
                        if isinstance(exp_content, list):
                            if exp_content not in self.quests:
                                self.testquests.append(exp_content)
                                self.label_testquest[label_id].append(exp_content)
                        else:
    ...
    def get_data_pre(self):
        ...
        '''
        if 1 == self.args.use_cut_words:
            self.label_quest = defaultdict(list)
            self.quests = list()
        else:
        ...
        if 1 == self.args.get_data_method:
            ...
            if 1 == self.args.use_cut_words:
                self.label_testquest = defaultdict(list)
                self.testquests = list()
            else:
...
#                allquest += quest
    ...
    def extend_by_sameword(self,quest):
        ...
        #扩充语句
        if 1 == self.args.use_cut_words:
            quests = self.extend_quests_use_fill_words(local_sentences)
        else:
            quests = self.extend_quests(quest, local_sentences)
        return quests
    
    def fill_empty_dict(self, fill_map, fills):
        if not fill_map.keys():
            for fill in fills:
                fill_map[fill] = {}
        else:
            for value in fill_map.values():
                self.fill_empty_dict(value, fills)
    
    def get_fill_list(self, output, string, fill_map):
        if fill_map.keys():
            for key, value in fill_map.items():
                self.get_fill_list(output, string+key+'#', value)
        else:
            output.append(string)
    
    def get_words_set(self, fill_list):
        fill_map = {}
        output = []
        for index, fills in enumerate(fill_list):
            self.fill_empty_dict(fill_map, fills)
        self.get_fill_list(output, '', fill_map)
        return output
    
    def extend_quests_use_fill_words(self, local_sentences):
        fill_list = []
        fill_index = []
        tmp = (['']*len(local_sentences))
        
        for index, words in enumerate(local_sentences):
            if not isinstance(words, list):
                tmp[index] = words
            else:
                fill_index.append(index)
                fill_list.append(words)
        if fill_list:
            tmpoutputs = self.get_words_set(fill_list)
            outputs = []
            for tmpoutput in tmpoutputs:
                output = tmpoutput.split('#')
                for incindex, index in enumerate(fill_index):
                    tmp[index] = output[incindex]
                if tmp not in outputs:
                    outputs.append(copy.deepcopy(tmp))
            return outputs
    ...
    def prepare_get_batch_data(self, addtional=0):
        labels = []
        if 0 == addtional:
            if isinstance(self.quests, list):
                quests = np.array(self.quests)
                for quest in self.quests:
                    for key, value in self.label_quest.items():
                        if quest in value:
                            labels.append(key)
                labels = np.array(labels)
                if len(labels) != len(self.quests):
                    self.print_log('labels:{}, quests:{}'.format(len(labels), len(self.quests)))
                    return None
            else:
                quests, labels = np.array(list(self.quest_label.keys())), np.array(list(self.quest_label.values()))
        elif 1 == addtional:
            if isinstance(self.testquests, list):
                quests = np.array(self.testquests)
                for quest in self.testquests:
                    for key, value in self.label_testquest.items():
                        if quest in value:
                            labels.append(key)
                labels = np.array(labels)
                if len(labels) != len(self.testquests):
                    self.print_log('labels:{}, quests:{}'.format(len(labels), len(self.testquests)))
                    return None
            else:
                quests, labels = np.array(list(self.testquest_label.keys())), np.array(list(self.testquest_label.values()))
        return quests, labels
    
    def get_batch_data(self, test_ratio, batch_size, num_epochs, startpos=5000):
        """生成批次数据"""
        quests, labels = self.prepare_get_batch_data()
        startpos = len(quests) - int(len(quests) * test_ratio)
        if 0 == self.args.get_batch_method:
            ...
            testquests, testlabels = self.prepare_get_batch_data(addtional=1)
            shuffle_indices = np.random.permutation(np.arange(len(quests)))#下标
            shuffle_indices_test = np.random.permutation(np.arange(len(testquests)))
            #是否打乱顺序
            train_x, train_y = quests[shuffle_indices], labels[shuffle_indices]#train
